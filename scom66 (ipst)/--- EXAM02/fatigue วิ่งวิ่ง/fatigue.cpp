...
void build2(int n, int a, int b) {
    ...
    build2(2*n,a,(a+b)/2);
    build2(2*n+1,(a+b)/2+1,b);
    tree2[n].first = min(tree2[2*n].first,tree2[2*n+1].first);
    tree2[n].second = max(tree2[2*n].second,tree2[2*n+1].second);
    return;
}

pair<int,int> query2(int n, int a, int b, int x, int y) {
    if (a > b || b < x || y < a)return {INT_MAX,INT_MIN};
    if (x <= a && b <= y) {
        return tree2[n];
    }
    pair<int,int> q1 = query2(2*n,a,(a+b)/2,x,y);
    pair<int,int> q2 = query2(2*n+1,(a+b)/2+1,b,x,y);
    pair<int,int> res;
...
long long interval_fatigue(std::vector<std::vector<int>> In){
    ...
    for (auto it : In) {
        int x = it[0];
        int y = it[1];
        pair<int,int> res = query2(1,0,n-1,x,y);
        lside = min(lside,res.first);
        rside = max(rside,res.second);
    }
    lguy = leftmost[lside];
    rguy = rightmost[rside];
    int l = lguy;
